...
A small, dependency-free command-line tool for exploring a dataset of trees.
It reads `trees_data.csv` (200 trees) and lets you look up individual records,
draw random samples, filter by criteria, and compute summary statistics
...
Filter options: `--type`, `--min-girth`, `--max-girth`, `--min-age`,
`--diseased`, `--healthy`, `--field`.
